...
import { Mic, MicOff, Volume2, VolumeX, CheckCircle, AlertCircle, Loader2, Sparkles, Globe, Brain } from "lucide-react"
import { useAuth } from "@/lib/auth-context"

interface AIOnlyFormFillerProps {
...
export default function AIOnlyFormFiller({ onComplete, onBack }: AIOnlyFormFillerProps) {
  const { user } = useAuth()
  ...
  const [step, setStep] = useState<"initial" | "processing" | "questions" | "complete">("initial")

  const recognitionRef = useRef<any>(null)
  const ttsService = useRef<any>(null)

  useEffect(() => {
    // Initialize Web Speech API with multiple language support
  ...
  const startRecording = async () => {
    ...
    try {
      if (recognitionRef.current) {
        // Set language based on detected language from AI result
        ...
        if (aiResult?.detected_language) {
          ...
          // Determine language from question text
          if (currentQuestion.includes("What") || currentQuestion.includes("your")) {
            lang = "en-US"
          } else if (currentQuestion.includes("మీ") || currentQuestion.includes("ఏమిటి")) {
            lang = "te-IN"
          } else if (currentQuestion.includes("என்") || currentQuestion.includes("உங்கள்")) {
            lang = "ta-IN"
          } else if (currentQuestion.includes("আপনার") || currentQuestion.includes("কী")) {
            lang = "bn-IN"
          } else if (currentQuestion.includes("તમારું") || currentQuestion.includes("શું")) {
            lang = "gu-IN"
          } else if (currentQuestion.includes("ನಿಮ್ಮ") || currentQuestion.includes("ಏನು")) {
            lang = "kn-IN"
          } else if (currentQuestion.includes("നിങ്ങളുടെ") || currentQuestion.includes("എന്ത്")) {
            lang = "ml-IN"
          } else if (currentQuestion.includes("ਤੁਹਾਡਾ") || currentQuestion.includes("ਕੀ")) {
            lang = "pa-IN"
          } else if (currentQuestion.includes("तुमचे") || currentQuestion.includes("काय")) {
            lang = "mr-IN"
          }
        }
          ...
          try {
            ...
            if (response.ok) {
              ...
              if (result.missing_required_fields && result.missing_required_fields.length > 0) {
                // Use the suggested questions (which should be in the detected language)
                const question = result.suggested_questions && result.suggested_questions.length > 0 
    ...
    try {
      ...
      const response = await fetch(`${API_BASE_URL}/translate-and-fill`, {
        ...
        body: JSON.stringify({
          ...
          source_language: aiResult.detected_language || "hi"
        })
      })
      ...
      if (response.ok) {
        ...
        if (result.translated_value) {
          console.log(`[DEBUG] Processing answer for field: ${missingFields[currentFieldIndex]}`)
            ...
            <div className="p-8 bg-gray-50 border-t border-gray-200 flex justify-between">
              ...
              {formData && Object.keys(formData).length > 0 && step === "complete" && (
                <button
                  ...
                  className="px-8 py-4 bg-gradient-to-r from-green-600 to-emerald-600 text-white rounded-xl hover:from-green-700 hover:to-emerald-700 transition-all flex items-center gap-3 font-semibold shadow-lg disabled:opacity-50"
                >
                  {isProcessing ? (
                  ...
                  {isProcessing ? "Submitting..." : "Complete Form"}
                </button>
              )}
            </div>
          </div>
