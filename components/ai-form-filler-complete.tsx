...
interface AIFormFillerCompleteProps {
  onComplete: (formData: Record<string, string>) => void
}

export default function AIFormFillerComplete({ onComplete }: AIFormFillerCompleteProps) {
  ...
  useEffect(() => {
    // Initialize Web Speech API with proper language support
  ...
  const startRecording = async () => {
    ...
    try {
      if (recognitionRef.current) {
        // Set language based on current question or user preference
        if (currentQuestion && aiResult?.detected_language) {
          ...
          recognitionRef.current.lang = langMap[aiResult.detected_language] || "hi-IN"
        }
        ...
        recognitionRef.current.onresult = async (event: any) => {
          ...
          try {
            ...
            // Process the initial speech with AI
